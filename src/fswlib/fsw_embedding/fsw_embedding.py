...
This code is based on the paper [FSW25]:
...
class FSWEmbedding(nn.Module):
    ...
    embedding as described in [FSW25].
    ...
    def __init__(self,
        ...
        total_mass_encoding_scale : float, default=1.0
            Scaling factor by which to multiply the encoded total mass before incorporating into the embedding.  
            Only applicable if `encode_total_mass`=True.  
            (See `TotalMassEncodingMethod` for details.)
        ...
        report : bool, default=False
            If True, prints a report with diagnostic information during initialization and forward computation.
        ...
        bias : torch.nn.Parameter or None
            ...
            Present only when ``enable_bias=True``; otherwise set to ``None``.
